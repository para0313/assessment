...
// X投稿ボタンの作成
  tweetDivision.innerText = ''; //ツイートのdiv空にする
  const ancher = document.createElement('a');// aタグの作成
  const hrefValue = 'https://x.com/intent/tweet?button_hashtag=' + encodeURIComponent('あなたのいいところ') + '&ref_src=twsrc%5Etfw';
 
ancher.setAttribute('href', hrefValue); // 属性hrefを追加
ancher.setAttribute('class','twitter-hashtag-button');
ancher.setAttribute('data-text', result);//診断結果を追加
ancher.innerText = '#あなたの良いところを投稿する｡';//ボタンの文章

tweetDivision.appendChild(ancher); // divの子要素を追加


const script = document.createElement('script');//scriptタグを作成
script.setAttribute('src','https://platform.x.com/widgets.js');//src属性を追加
tweetDivision.appendChild(script);//divの子要素を追加
  }
);
...
userNameInput.addEventListener(
  ...
  (event) => {
    if(event.code === 'Enter'){
      assessmentButton.dispatchEvent(new Event('click'));//TODO Enter キーが押されたときに実行する処理
    }
  }
)
